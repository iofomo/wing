...
`repo`采用`Python`开发语言实现，真正的功能对应于一个`repo`代码库，其管理多个`git`库代码的本地批处理工具，工作原理如下：
...
在分支开发，测试稳定后，合并至主干`master`分支，其他人通过`repo sync`命令同步至最新版本，即可使用。 <span style="color:blue;">【通常管理员维护】</span>

#### doc

该目录为本工程的内部开发组的文档存放目录，如：产品线或本项目设计和开发文档等。可以通过命令`repo -refresh`生成文档索引。[详情](#jump-refresh)

#### out

该目录为各功能编译后输出结果文件目录，自动创建，输出的内容存放保持工程目录结构一致。通常通过命令`repo -build`编译生成。
...
2）依赖当前`REPO`工程的其他`git`库模块，采用本地输出文件依赖
...
3）依赖其他`REPO`工程中`git`模块的，则统一采用`maven/cocopods`依赖
...
各`git`模块编译结果均输出至`REPO`的`out`目录（即：`${REPO root path}/out`），模块之间依赖的文件也均从该`out`目录下获取。
...
# 如Android平台开发人员同步代码为：
$ python repo.py init project-a zhangsan master android.xml

# 如iOS平台开发人员同步代码为：
$ python repo.py init project-a zhangsan master ios.xml

# 如服务端后台(若管理员未单独配置前端的web.xml则通用此索引)平台开发人员同步代码为：
$ python repo.py init project-a zhangsan master server.xml

# 如PC平台开发人员同步代码为：
$ python repo.py init project-a zhangsan master pc.xml

# 如测试开发人员同步代码为：
$ python repo.py init project-a zhangsan master test.xml
```
...
# 下面以代码组名称为：project-a，用户名为：zhangsan，代码组分支为：master，为例
# 如Android平台开发人员同步代码为：
$ repo init project-a zhangsan master android.xml

# 如iOS平台开发人员同步代码为：
$ repo init project-a zhangsan master ios.xml

# 如服务端后台(若管理员未单独配置前端的web.xml则通用此索引)平台开发人员同步代码为：
$ repo init project-a zhangsan master server.xml

# 如PC(Windows/Linux/MacOS客户端开发)平台开发人员同步代码为：
$ repo init project-a zhangsan master pc.xml

# 如测试开发人员同步代码为：
$ repo init project-a zhangsan master test.xml
```
...
在代码组根目录执行`repo -build`，则编译整个代码组。

#### 创建

创建开发工程（在repo工程目录下任意位置，执行如下命令，可以一键创建开发工程）
...
$ repo -create jar demo

# 创建Android的平台工程
$ repo -create ap demo
# 创建Android的平台工程(该工程包含native模块)
$ repo -create anp demo
...
<span style="color:red;">【慎用】</span>在repo工程目录下执行如下命令，则会遍历该repo工程目录下所有工程的缓存文件清除（Jenkins编译默认启用）

```shell
# 遍历repo工程目录下的所有子Android/iOS工程，清除gradle/xcode下载的依赖模块的缓存
$ repo -clean gradle
# 遍历repo工程目录下的所有git工程，清除除了“develop”，“master”和当前分支外的所有本地分支，清除git缓存
$ repo -clean git
# 遍历repo工程目录下的所有Python工程，清除编译缓存（*.pyc/*.pym/*.pyo等）
$ repo -clean py
```
...
```shell
# 格式为：repo -adb pull {package name}，如：
$ repo -adb pull com.demo
# 输出结果为：
...
执行成功后，并输出结果：`${REPO ROOT PATH}/out/project.json`。
...
$ repo -key list com.demo.apk
#【Android】查看签名证书的信息，无密码，需要两次回车，即输入密码时忽略
$ repo -key list debug.keystore
#【Android】查看签名证书的信息，带密码
$ repo -key list debug.keystore 123456

#【iOS】查看安装包文件的签名信息（解析Payload/Demo.app/embedded.mobileprovision）
$ repo -key list demo.ipa
#【iOS】查看p12的签名信息
$ repo -key list debug.p12 xxxxxx 123456
#【iOS】查看签名文件信息
$ repo -key list embedded.mobileprovision
```
...
```shell
# 命令格式为：
$ repo -newbranch [branch name] [new branch name]
# 如：
$ repo -newbranch develop release_v1.0
```
...
```shell
# 命令格式为：
$ repo -newtag [branch name] [new tag name]
# 如：
$ repo -newtag develop tag_v1.0
```

#### 提交代码

>在`REPO`工程的<span style="color:blue;">任何Git工程</span>目录下执行此命令，则会提交当前Git工程代码至远程服务器
...
><span style="color:red;">【慎用】</span>在`REPO`工程根目录下执行此命令，则会遍历当前所有代码组，提交未`push`至远程服务的代码
