...
def run():
    ...
    # wing -cocollider scan ${path}
    # wing -cocollider run ${file}
    ...
    if cmd == 'scan':
        group = CocoGroup()
        path = argv.get(3)
        if CmnUtils.isEmpty(path): path = envPath
        doScan(path, group)
        doPrintln(envPath, group)
        return
    # "wing -coco run ${type}"
    if cmd == 'run':
        doRunner(argv.get(3), envPath)
        return

    print('Invalid command:')
    print('    wing -cocollider scan')
    print('    wing -cocollider scan ${path}')
    print('    wing -cocollider run ${file}')
